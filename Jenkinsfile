    ...
    stages {
        ...
        stage('Deploy to AWS EC2') {
            steps {
                script {
                   bat '''
                    ssh -o StrictHostKeyChecking=no -i C:\\Users\\mingx\\.ssh\\DevOpsDeploy.pem ec2-user@54.167.205.236 "cd ~/ansible-project && ansible-playbook -i inventory.ini deploy-app.yml"
                    '''
                }
            }
        }
